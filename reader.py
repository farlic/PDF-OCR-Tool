...
months = {
	...
	"november":"11",
	"december":"12"}
class Error(Exception):#base clas for other exceptions
	pass
...
def convert_month(month_string:str):
	if "!" in month_string:
		month_string.replace("!","l")
	try:
		output_month = months[month_string]
		return output_month
	except:
		raise DateError

def convert_day(day_string:str):
	if day_string.isdecimal() and len(day_string) == 2:
		return day_string
	try:
		output = days[day_string]
		return output
	except:
		raise DateError
...
print('pdf loaded')
print(f'load time: {round(time.time() - load_time)}s')
wb = Workbook()
ws = wb.active
ws.title = "data"
...
for index,page in enumerate(pages):
	...
	os.system('cls')
	title()
	print(f'processing page {index+1} of {len(pages)}')
	...
	try:
		...
		if regex.search(r'(certificate received){e<3}',text_full.lower()):
			print('coroner found')
			p.informant_name = ''
			p.informant_surname = ''
			p.informant_midname = ''
			p.informant_address = '' 
			p.qualification = 'coroner'

		if not regex.search(r'(certificate received){e<3}',text_full.lower()):
			qualification_span = []
			...
			qualification_clean = p.qualification.split(' ')
			if len(qualification_clean)>1:
				if 'widow' in p.qualification.lower():
					p.qualification = 'widow'
				elif 'widower' in p.qualification.lower():
					p.qualification = 'widower'
					
				elif "law" in p.qualification:
					pass
				else:
	...
	except (AttributeError,ValueError,DateError) as e:
		error_list.append(f'{index+1}')
...
print(f'--------------------------------------------------')
print(f'average reading time: {round(sum(times)/len(times))}s')
print(f'--------------------------------------------------')
